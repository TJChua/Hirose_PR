...
namespace BSI_PR.Module.Controllers
{
    ...
    public partial class JapanPODetailsViewControllers : ObjectViewController<ListView, PurchaseOrderJapanDetails>
    {
        public JapanPODetailsViewControllers()
        {
            InitializeComponent();
        ...
        {
            if (e.CreatedObject is PurchaseOrderJapanDetails && View.IsRoot == false)
            {
                PurchaseOrderJapanDetails currentObject = (PurchaseOrderJapanDetails)e.CreatedObject;
                ...
                if (lv.CollectionSource is PropertyCollectionSource)
                {
                    ...
                    {
                        if (collectionSource.MasterObjectType == typeof(PurchaseOrderJapan))
                        {
                            int i = 0;
                            PurchaseOrderJapan masterobject = (PurchaseOrderJapan)collectionSource.MasterObject;

                            foreach (PurchaseOrderJapanDetails dtl in masterobject.PurchaseOrderJapanDetails)
                            {
                                if (i == 1)
                                {
                                    return;
